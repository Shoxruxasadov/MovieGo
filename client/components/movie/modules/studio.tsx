...
export default function StudioMoviesCollection({ path, module }: { path: string, module: string }) {
    ...
    const { data, error, fetchNextPage, hasNextPage, isFetching, isFetchingNextPage, status } = useInfiniteQuery<MoviesResponse>({
        queryKey: [`${module}-movies`],
        queryFn: ({ pageParam = 0 }) => api.get<MoviesResponse>(`/${module}/${path}?page=${pageParam}`).then(({ data }) => data).catch(error => { throw error }),
        initialPageParam: 0,
